...
def Loc_Trung_binh_so_hoc(img, ksize):
    ...
    h=(ksize -1) // 2           #chia lấy phần nguyên
...
if __name__ == "__main__":
    ...
    for name in lists:
        ...
        opening = cv2.morphologyEx(img_ket_qua_TBSH, cv2.MORPH_OPEN, kernel)

        PSNR_noise = PSNR(img_original, img_nhieu)
        PSNR_denoise1 = PSNR(img_original, img_ket_qua_TBSH)
        ...
        SSIM_denoise1 = ssim(img_original, img_ket_qua_TBSH)
        ...
        ax3.imshow(img_ket_qua_TBSH, cmap='gray')  # Hiển thị ảnh sau khi lọc
        ax3.set_title("ảnh sau khi lọc trung bình số học" + "(" + f'PSNR denoise: {round(PSNR_denoise1, 3)}\n SSIM denoise: {round(SSIM_denoise1, 3)}' + ")")  # Thiết lập tiêu đề vùng ax2
